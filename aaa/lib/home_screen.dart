...
Timer? _historyTimer;
  ...
  _autoRefreshTimer =
      Timer.periodic(const Duration(seconds: 5), (_) => _fetchMyKeys());
  _newNotificationsTimer =
      Timer.periodic(const Duration(seconds: 5), (_) => _checkNewNotifications());
  _historyTimer =
      Timer.periodic(const Duration(minutes: 1), (_) => _fetchKeyHistory()); // ⏱️ раз в минуту
}
      ...
      return Center(
        child: Column(
          ...
          children: const [
            ...
            SizedBox(height: 16),
            Text("Сегодня вы не брали ключи",
                style: TextStyle(
  ...
  itemBuilder: (context, index) {
    ...
    if (!status && issueRecord != null) {
      ...
      final issuedAt = DateTime.tryParse(
        issuedAtStr.replaceAllMapped(RegExp(r"(\d{2})\.(\d{2})\.(\d{4}) (\d{2}):(\d{2})"), 
